# -*- coding: utf-8 -*-
"""
Created on Sat Dec 30 20:20:57 2023
...
# 打开数据库
def opendb():
        conn = sqlite3.connect("D:\work.0\python\mydb.db")
        cur = conn.execute("""create table if not exists tongxinlu\
                           (usernum integer primary key,username varchar(128), \
                            passworld varchar(128),address varchar(125), \
                                telnum varchar(128))""")
        return cur, conn
#查询全部信息
def showalldb():
...
def adddb():
        ...
        hel[1].execute("insert into tongxinlu(usernum,username, passworld, address,\
                       telnum)values (?,?,?,?,?)",(person[0], person[1],\
...
def alter():
        ...
        hel =opendb()
        person = into()
        hel[1].execute("update tongxinlu set usernum=?,username=?, passworld= ?,\
                       address=?,telnum=? where usernum="+changechoice,(person[0], \
